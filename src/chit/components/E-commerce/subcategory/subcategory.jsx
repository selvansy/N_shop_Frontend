...
const SubCategory = () => {
  ...
  const { mutate: getCategory } = useMutation({
    mutationFn: (payload) => getsubcategoryTable(payload),
    onSuccess: (response) => {
  ...
  const { mutate: deleteCategory } = useMutation({
    mutationFn: ({ CategoryId }) => deletesubcategory(CategoryId),
    onSuccess: (response) => {
  ...
  const handleStatusToggle = async (id) => {
    let response = await activatesubcategory(id);
    if (response) {
      ...
      cell: (row) => {
        ...
        return (
          <div
            ...
            title={desc} // Tooltip shows full text on hover
          >
            {!desc || desc === "null" ? "-" : desc}
          </div>
        );
    ...
    // },
      ...
      cell: (row) => (
        <div className="w-12 h-12 rounded overflow-hidden">
          <img
            src={`${row?.pathurl}${row?.bannerImage}`}
            alt={ "Preview"}
  ...
  return (
    <>
      <Breadcrumb
        items={[{ label: "Catalogue" }, { label: "Sub Category", active: true }]}
      />
      ...
      <div className="flex flex-col p-4  bg-white border border-[#F2F2F9]  rounded-[16px]">
        ...
        >
          <Addsubcategory setIsOpen={setIsviewOpen} id={id} clearId={clearId} />
        </ModelOne>
        <Modal />
      </div>
    </>
