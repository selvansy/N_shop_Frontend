...
import Addpincodemaster from "./addpincodemaster";
...
const Pincodemaster = () => {
  ...
  const { mutate: getCategory } = useMutation({
    mutationFn: (payload) => pincodetable(payload),
    onSuccess: (response) => {
  ...
  const { mutate: deleteCategory } = useMutation({
    mutationFn: ({ _id }) => deletestonemaster(_id),
    onSuccess: (response) => {
  ...
  const handleDelete = (id) => {
    ...
    dispatch(
      openModal({
        ...
        formData: {
          message:
            "Are you sure you want to delete this category? This will also delete all products related to this category, and this action cannot be undone.",
          _id: id,
        },
        buttons: {
          cancel: {
  ...
  const handleStatusToggle = async (id) => {
    let response = await activatepincode(id);
    if (response) {
      setPincodedata((prev) =>
        prev.map((cat) =>
  ...
  const columns = [
    ...
    // {
    //   header: "Country",
    //   cell: (row) => row?.id_country?.country_name,
    // },
    {
      header: "State",
      cell: (row) => row?.id_state?.state_name,
    },
    {
      header: "City",
      cell: (row) => row?.id_city?.city_name,
    },
    {
      header: "Pincode",
      cell: (row) => row.pincode || "N/A",
    },
    // {
    //   header:"Delivery Available",
    //   cell:(row)=> row.isDeliveryAvailable || "N/A"
    // },
    {
      header: "Delivery Available",
      cell: (row) =>
        row.isDeliveryAvailable === true
          ? "Yes"
          : row.isDeliveryAvailable === false
          ? "No"
          : "N/A",
    },
    {
      header: " Expected Delivery",
      cell: (row) => row.estimatedDeliveryDays || "N/A",
    },
    {
  ...
  return (
    <>
      ...
      <div className="flex flex-col p-4  bg-white border border-[#F2F2F9]  rounded-[16px]">
        ...
        <ModelOne
          title={id ? "Edit Pincode " : " Create Pincode"}
